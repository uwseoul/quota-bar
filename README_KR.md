...
macOS 메뉴 바에서 **AI 코딩 플랜의 토큰 소모 속도**를 여러 플랫폼에서 실시간으로 모니터링하는 가볍고 강력한 앱입니다.

**차별점**: 단순한 사용량 퍼센트가 아니라, **남은 시간 대비 현재 소모 속도가 얼마나 빠른지**를 보여줍니다. 초과하기 전에 미리 알 수 있습니다.
...
- **🔥 토큰 소모 속도 모니터링 (Pace)**: 이 앱의 핵심 기능 — 단순히 얼마나 썼는지가 아니라, **남은 기간 대비 지금 소모 속도가 평균보다 빠른지 느린지**를 실시간으로 보여줍니다. 초록색은 여유, 노란색은 적정, 빨간색은 초과 위험.
...
- **3가지 표시 모드**:
  ...
  - **속도 신호등(Pace)**: 토큰 소모 속도를 신호등으로 표시 (녹/노/빨)
- **4가지 메뉴 바 모드**:
...
- **소모 속도 기반 색상 표시**:
  - 🔴 빨강 (Fast): 소모 속도가 빠름 — 할당량 초과 위험
  - 🟡 노랑 (Normal): 적정 소모 속도
  - 🟢 초록 (Slow): 여유 있는 소모 속도 — 안전
